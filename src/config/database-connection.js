...
function databaseInserts() {
    (async () => {
        ...
        const estadia1 = await Estadia.create({ checkIn: "2026-10-15", checkOut: "2026-10-20", valorTotalEstadia: 900.00, reservaId: reserva1.id, funcionarioId: f1.id });
        const estadia2 = await Estadia.create({ checkIn: "2026-09-15", checkOut: "2026-09-25", valorTotalEstadia: 2500.00, reservaId: reserva2.id, funcionarioId: f2.id });
        const estadia3 = await Estadia.create({ checkIn: "2026-06-02", checkOut: "2026-06-30", valorTotalEstadia: 420.00, reservaId: reserva3.id, funcionarioId: f3.id });
        const estadia4 = await Estadia.create({ checkIn: "2026-04-30", checkOut: "2026-05-02", valorTotalEstadia: 1000.00, reservaId: reserva4.id, funcionarioId: f4.id });
        
        const q1 = await Quarto.create({ andar: 1, nome: "Quarto 101", tipoDeQuartoId: 1, estadiaId: 1 });
        const q2 = await Quarto.create({ andar: 2, nome: "Quarto 201", tipoDeQuartoId: 2, estadiaId: 2 });
        const q3 = await Quarto.create({ andar: 3, nome: "Quarto 302", tipoDeQuartoId: 3, estadiaId: 3 });
        const q4 = await Quarto.create({ andar: 4, nome: "Quarto 405", tipoDeQuartoId: 4, estadiaId: 4 });
